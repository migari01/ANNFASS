...
class ColorFromTexture:
    def __init__(self, filename, OBJFolder, TextureFolder, PLYFolder, MTLFolder):
        self.vertices = []
        self.textures = []
        self.bary_coord = []
        self.face = {}
...
#                        ni = (float)(line.split()[1])
    ...
    def getFaceInfo(self):
        ...
        for line in fread:
            ...
            if line.startswith("v "):
                ...
                self.face[self.faceid] = {"v":vertex, "tex":texture, "mat":material}
                self.faceid += 1
    ...
    def collectMetadataOfPoints(self):
        fread = open(os.path.join(self.PLYFolder, self.fname+".ply"))
...
def getColorFromTextureFromAList(inputfile, objFolder, textureFolder, PLYFolder, MTLFolder, destinationFolder):
    fread = open(inputfile, 'r')
    for line in fread:
        line = line.strip()
        getColorFromTextureOfAFile(line, objFolder, textureFolder, PLYFolder, MTLFolder, destinationFolder)
    fread.close()
