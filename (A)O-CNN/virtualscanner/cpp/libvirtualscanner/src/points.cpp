...
void PtsInfo::set_channel(PropType ptype, const int ch) {
  ...
  int i = property_index(ptype);
  channels_[i] = ch;
  content_flags_ |= ptype;//bitwise or, acts like +=
}
...
bool Points::write_points(const string& filename) const {
  ...
  outfile.close();
  //cout<< "in write points"<<endl;
  //info_->write_info(filename);
  return true;
}

bool Points::write_ply(const string & filename, bool f, bool l) const {
  std::ofstream outfile(filename, std::ios::binary);
  if (!outfile) return false;
  ...
  if(l)
    outfile<<"property uchar label"<<std::endl;
  ...
  const float* labels;
  if (f)
    feat=ptr(PtsInfo::kFeature);
  if(l)
    labels=ptr(PtsInfo::kLabel);
  ...
  for (int i = 0; i < n; ++i) {
    ...
    sprintf(pstr + i * len,
      ...
      sprintf(pstr + i * len,
          "%.6f %.6f %.6f %.6f %.6f %.6f %d %d %d %d\n",
          pts[3 * i], pts[3 * i + 1], pts[3 * i + 2],
          normals[3 * i], normals[3 * i + 1], normals[3 * i + 2],
          int(feat[4*i]),int(feat[4*i+1]),int(feat[4*i+2]),int(feat[4*i+3]));
    }
    else if(!f & l){
	sprintf(pstr + i * len,
        "%.6f %.6f %.6f %.6f %.6f %.6f %d\n",
        pts[3 * i], pts[3 * i + 1], pts[3 * i + 2],
        normals[3 * i], normals[3 * i + 1], normals[3 * i + 2],int(labels[i]));
    }
    else{
      sprintf(pstr + i * len,
          "%.6f %.6f %.6f %.6f %.6f %.6f %d %d %d %d %d\n",
          pts[3 * i], pts[3 * i + 1], pts[3 * i + 2],
          normals[3 * i], normals[3 * i + 1], normals[3 * i + 2],
          int(feat[4*i]),int(feat[4*i+1]),int(feat[4*i+2]),int(feat[4*i+3]),
          int(labels[i]));
...
bool Points::write_txt(const string & filename, bool f, bool l) const {
  std::ofstream outfile(filename, std::ios::binary);
  if (!outfile) return false;
...
//  }
//}
bool Points::createHeader(const string& filename){
  std::ifstream infile(filename, std::ios::binary);
  cout<<filename<<endl;
  if (!infile) return false;
  string out_path  = filename.substr(0, filename.rfind('.'));
  out_path+="_w_header.points";
